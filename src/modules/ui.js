...
export default class UI {
    ...
    attachEvents = () => {
        ...
        this.themeBtn.addEventListener('click', this.handleThemeClick);
        this.todoInput.addEventListener('keypress', this.handleKeyPress);
    }

    subscribe = () => {
        pubsub.subscribe("listChanged", this.renderList);
        pubsub.subscribe("themeChanged", this.changeTheme);
    }
    ...
    createTodoElement = (newTodo) => {
        ...
        <li class="todo-item ${newTodo.isComplete ? "todo-item--complete" : ""}" data-id="${newTodo.id}">
            <div class="check-text-wrapper">
                <button class="todo-item__button todo-item__button--checkbox" ><input type="checkbox" ${newTodo.isComplete ? "checked" : ""} class="todo-item__checkbox" />
                </button>
        ...
        return todoElement.childNodes[1];
    }

    changeTheme = (currTheme) => {
        this.body.className = currTheme;
    }
}